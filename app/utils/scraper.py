...
class ScraperUtils:
    ...
    async def login(self):
        ...
        if response_login.status_code != 200:
            account: Account = await account_controller.get_account_by_account_number(self.username)
            account_controller.update(id=account.id, obj_in={"feedback": "登陆异常请重试!"})
            return False, ""
    ...
    async def get_license(self, index_text):
        ...
        if len(url) == 0:
            account: Account = await account_controller.get_account_by_account_number(self.username)
            account_controller.update(id=account.id, obj_in={"feedback": "账号密码错误,请修改后重试！"})
            return False, ""
        response_license = self.session.get(url[0], headers=self.headers, verify=False)
        if response_license.status_code != 200:
            account: Account = await account_controller.get_account_by_account_number(self.username)
            account_controller.update(id=account.id, obj_in={"feedback": "采集异常,请手动查看后重试!"})
            return False, ""
    ...
    async def get_mor9(self, mor5_text, create_by: str):
        tree = etree.HTML(mor5_text)
        if len(tree.xpath("//*[@id='top']/div/nav/div[2]/ul/li[7]/ul/li[1]/a/@href")) == 0:
            account: Account = await account_controller.get_account_by_account_number(self.username)
            account_controller.update(id=account.id, obj_in={"feedback": "账号密码错误,请修改后重试！"})
        ...
        for tr in tr_list:
            ...
            if mor:
                ...
                if item["MOR9_STATUS"] != mor.apply_status:
                    await sendEmail(user=create_by, result=mor_obj)
            else:
...
if __name__ == '__main__':
    scraper = ScraperUtils(username='0105548160264', password='K@ming0101')
    # scraper = ScraperUtils(username='0105548160264', password='12345')
    flag, response = asyncio.run(scraper.login())
    retry, response_l = asyncio.run(scraper.get_license(response))
    asyncio.run(scraper.get_mor9(response_l, "admin"))
